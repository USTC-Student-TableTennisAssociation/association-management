...
function EmptyState({ mode, onCreate, onInvokeAI }: { mode: StudioMode; onCreate: () => void; onInvokeAI: () => void }) {
  return <div className={styles.emptyState}><span><Icon name={mode === "method" ? "book" : "map"} /></span><p>{mode === "method" ? "组织方法" : "任务版图"}</p><h2>{mode === "method" ? "把经验整理成可以复用、展开和套用的流程。" : "先建立一次真实活动，再把所有工作放进同一张依赖图。"}</h2><small>{mode === "method" ? "方法本身不等于本届进度；套用后才会生成正式工作包和任务。" : "版图同时呈现工作包、任务、负责人、期限和前置关系。"}</small><div><button type="button" onClick={onCreate}><Icon name="plus" />直接建立</button><button type="button" onClick={onInvokeAI}><Icon name="spark" />让 Sydaris 协助</button></div></div>;
}
