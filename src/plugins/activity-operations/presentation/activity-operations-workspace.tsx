...
function EmptyState({ mode, onCreate, onAskAI }: { mode: StudioMode; onCreate: () => void; onAskAI: () => void }) {
  return <div className={styles.emptyState}><span><Icon name={mode === "method" ? "book" : "map"} /></span><p>{mode === "method" ? "组织方法" : "任务版图"}</p><h2>{mode === "method" ? "把经验整理成可以复用、展开和套用的流程。" : "先建立一次真实活动，再把所有工作放进同一张依赖图。"}</h2><small>{mode === "method" ? "方法本身不等于本届进度；套用后才会生成正式工作包和任务。" : "版图同时呈现工作包、任务、负责人、期限和前置关系。"}</small><div><button type="button" onClick={onCreate}><Icon name="plus" />直接建立</button><button type="button" onClick={onAskAI}><Icon name="spark" />让 Echo 协助</button></div></div>;
}

function CardInspector({ model, mode, selectedId, objectNames, saving, onEdit, onAddTask, onAddDependency, onRemoveDependency, onSetNested, onRemoveEdge, onAskAI }: {
  ...
  onRemoveEdge: (edge: MethodEdge) => void; onAskAI: (card: ViewCardState) => void;
}) {
  if (mode === "method") {
...
export function ActivityOperationsWorkspace({ viewKey, refreshRevision = 0, focusCardId, onOpenInspector, onAskAI }: WorkspaceProps) {
  ...
  const askAbout = (card: ViewCardState) => onAskAI(`请读取 ${viewKey} 中 ID 为 ${card.id} 的“${text(card, "name") ?? card.cardTypeKey}”，结合当前正式状态检查信息缺口、依赖和下一步。需要写入时请先提交可审批命令。`);
