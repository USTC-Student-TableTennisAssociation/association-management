...
describe("Echo Markdown debug trace", () => {
  it("is enabled by default outside tests and can be explicitly disabled", () => {
    ...
    expect(debugTraceEnabled({
      ...
      ECHO_DEBUG_TRACE: "false",
    } as NodeJS.ProcessEnv)).toBe(false);
    expect(debugTraceEnabled({
      NODE_ENV: "test",
      ECHO_DEBUG_TRACE: "true",
    } as NodeJS.ProcessEnv)).toBe(true);
  });

  it("writes one readable Markdown file and redacts secrets", async () => {
    const root = path.join(os.tmpdir(), `echo-debug-trace-${crypto.randomUUID()}`);
    const environment = {
      NODE_ENV: "test",
      ECHO_DEBUG_TRACE: "true",
      ECHO_DEBUG_TRACE_DIR: root,
      AI_API_KEY: "sk-secret-example-value",
      DATABASE_URL: "postgresql://user:password@localhost/database",
    } as NodeJS.ProcessEnv;
    const trace = createEchoDebugTrace({
    ...
    expect(markdown).toContain("# Echo Chat 调试报告");
  ...
  it("redacts bearer tokens and Postgres URLs without configured env values", () => {
    const redacted = redactDebugSecrets(
      "Bearer abcdefghijklmnopqrstuvwxyz postgresql://user:pass@db.example/echo",
    );
