...
function RunRow({ run, publicationPending = false }: {
  ...
  const locationLabel = run.originalRelativePath && run.originalRelativePath !== run.nodeName
    ? run.originalRelativePath
    : run.mimeType;
  return (
    <div className="grid gap-2 border-b border-zinc-100 px-4 py-3 last:border-b-0 lg:grid-cols-[minmax(0,1fr)_8rem_8rem]">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 truncate text-xs text-zinc-400" title={locationLabel}>{locationLabel}</p>
      ...
      <div className="text-xs text-zinc-600">
        {run.profile === "deep" && run.resultSummary ? (
          <p className="text-zinc-500">{run.resultSummary}</p>
        ) : run.assessment ? (
...
export function CompilationWorkspace({
  ...
  const admissionUnits = deepParallelUnits.filter((unit) => unit.kind === "object_admission");
  ...
  async function action(actionName: "pause" | "resume" | "retry_failed" | "retry_global" | "recover_stale") {
            ...
            <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  {overview.job.failedContent > 0 && (overview.job.status === "failed" || overview.job.status === "paused") ? <button type="button" disabled={busy} onClick={() => void action("retry_failed")} className="rounded-md border border-red-200 px-3 py-2 text-sm text-red-700 disabled:opacity-50">重试失败项</button> : null}
                  {overview.job.failedContent === 0 && overview.job.status === "failed" && overview.job.globalResolution.status === "failed" ? <button type="button" disabled={busy} onClick={() => void action("retry_global")} className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-800 disabled:opacity-50">重试归并与发布</button> : null}
                </div>
              </div>
              <div className="mt-5">
                  ...
                  <div className="mt-4 rounded-lg border border-emerald-200 bg-white/70 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-zinc-500">来源节点 {sourceUnits.length}/{overview.job.concurrency.deepSources} 路活跃 · 模型在途上限 {overview.job.concurrency.coldStartModels} · Global Object {deepGlobalUnits.length}/{overview.job.concurrency.globalObjects} 路活跃 · 准入复审 {admissionUnits.length} 路活跃</p>
                      </div>
                    ...
                    {admissionUnits.length ? (
                      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                        {admissionUnits.map((unit) => (
                          <div key={`${unit.runId}:${unit.kind}:${unit.id}`} className="min-w-0 rounded-md border border-violet-200 bg-violet-50 px-3 py-2">
                            <p className="truncate text-xs font-semibold text-violet-900" title={unit.id}>Object 准入 · {unit.id}</p>
                            <p className="mt-1 line-clamp-2 text-xs leading-4 text-violet-800/70">{unit.statusMessage}</p>
                          </div>
                        ))}
                      </div>
                    ) : null}
