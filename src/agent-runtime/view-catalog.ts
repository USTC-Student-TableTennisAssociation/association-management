...
export function createViewCatalog(registry: ExtensionRegistry): ViewCatalogEntry[] {
  return registry.listViews().map((view) => ({
    ...
    aiWriteCapabilities: view.manifest.aiWriteCapabilities ?? [],
    dataBoundaries: view.manifest.dataBoundaries ?? [],
...
export function buildViewCatalogContext(registry: ExtensionRegistry): string {
  ...
  return [
    ...
    "具体写入能力以 Catalog 列出的 AI 可提议事项及激活 Skill 后实际开放的 Command 为准，不能从某个 View 支持 AI Command 推断任意 Card 都可创建或修改。",
    ...entries.flatMap((view) => [
      `- ${view.key}（${view.label}）${view.specializedLabel ? ` · ${view.specializedLabel}` : ""}：${view.description}`,
      `  适用任务：${view.retrievalDescription}`,
      ...
      `  专业查询：${view.queries.length
        ...
        : "无"}。`,
      `  AI 可提议：${view.supportsCommands
        ? view.aiWriteCapabilities.length
          ? view.aiWriteCapabilities.join("；")
          : "存在待审批 Command，具体能力需以激活 Skill 后实际开放的 Command 为准"
        : "无"}。`,
      ...(view.dataBoundaries.length
        ? [`  数据边界：${view.dataBoundaries.join("；")}。`]
        : []),
    ]),
  ].join("\n");
}
