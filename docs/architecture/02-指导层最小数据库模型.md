...
正式的数据库定义位于 [`prisma/schema.prisma`](../../prisma/schema.prisma)。填写真实 `DATABASE_URL` 后，通过 `pnpm prisma:migrate -- --name init_guidance_schema` 生成 Prisma 的第一份迁移；不再维护独立的手写 SQL Schema。
