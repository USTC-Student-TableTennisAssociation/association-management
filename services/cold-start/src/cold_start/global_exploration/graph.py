...
class GlobalExplorationRunner:
    ...
    def _graph(self):
        ...
        graph.add_node("compile_tree", self._compile_tree)
        graph.add_edge(START, "prepare")
        graph.add_edge("prepare", "document_context")
        graph.add_edge("prepare", "root_partition")
        graph.add_edge(["document_context", "root_partition"], "compile_tree")
        graph.add_edge("compile_tree", END)
    ...
    async def _compile_tree(self, state: State) -> State:
        document = state["document"]
        ...
        self.progress.report(
            "汇总",
            f"区域树状态 {tree.status}，叶子 {len(tree.leaf_node_ids)} 个",
        )
